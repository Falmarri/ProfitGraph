...
class ModelTest(QtCore.QObject):
    def __init__(self, _model, parent):
        ...
        self._model = _model
        self.model = _model#sip.cast(_model, QtCore.QAbstractItemModel)
        self.insert = []
        self.remove = []
        self.fetchingMore = False
        assert(self.model)
       
        self.connect( self.model, QtCore.SIGNAL("columnsAboutToBeInserted()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("columnsAboutToBeRemoved()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("columnsBeInserted()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("columnsRemoved()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("dataChanged()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("headerDataChanged()"), self.runAllTests)
        ...
        self.connect( self.model, QtCore.SIGNAL("rowsAboutToBeInserted()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("rowsAboutToBeRemoved()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("rowsBeInserted()"), self.runAllTests)
        self.connect( self.model, QtCore.SIGNAL("rowsRemoved()"), self.runAllTests)

        # Special checks for inserting/removing
        self.connect( self.model, QtCore.SIGNAL("rowsAboutToBeInserted()"), self.rowsAboutToBeInserted)
        self.connect( self.model, QtCore.SIGNAL("rowsAboutToBeRemoved()"), self.rowsAboutToBeRemoved)
        self.connect( self.model, QtCore.SIGNAL("rowsBeInserted"), self.rowsInserted)
        self.connect( self.model, QtCore.SIGNAL("rowsRemoved()"), self.rowsRemoved)
        self.runAllTests()
